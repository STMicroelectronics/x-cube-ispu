/**
  ...
  *                      of the STMicroelectronics.X-CUBE-ISPU.2.0.0 instances.
  ******************************************************************************
  * @attention
  *
...
static void ISPU_Handler(TMsg_t *Msg)
{
  ...
  BSP_SENSOR_ACC_Write_Register(LSM6DSO16IS_INT2_CTRL, 0x80);

  BSP_SENSOR_ACC_Read_Register(LSM6DSO16IS_ISPU_INT_STATUS0_MAINPAGE, &int_status);

  if (int_status != 0U)
  {
    BSP_SENSOR_ACC_Write_Register(LSM6DSO16IS_FUNC_CFG_ACCESS, 0x80);

    for (i = 0U; i < IspuBytes; i++)
    {
      BSP_SENSOR_ACC_Read_Register(LSM6DSO16IS_ISPU_DOUT_00_L + i, &ispu_status[i]);
    }

    BSP_SENSOR_ACC_Write_Register(LSM6DSO16IS_FUNC_CFG_ACCESS, 0x00);
  }
